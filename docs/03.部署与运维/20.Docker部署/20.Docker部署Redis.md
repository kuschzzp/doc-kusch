...
wget http://cdn.superkusch.fun/confs/redis.conf && \
...
> # redis8 之后需要 别用 requirepass，用下面这种写法
> user default on >Redis@123 allkeys allcommands
...
-v /opt/dockerAllHere/redis/conf/redis.conf:/usr/local/etc/redis/redis.conf \
-v /opt/dockerAllHere/redis/data:/data \
-d redis:8.4.0 redis-server /usr/local/etc/redis/redis.conf
```
...
-v /opt/dockerAllHere/redis/config/redis.conf:/usr/local/etc/redis/redis.conf：映射配置文件  
-d : 后台运行  
redis-server /usr/local/etc/redis/redis.conf : 配置文件方式启动  
:::
